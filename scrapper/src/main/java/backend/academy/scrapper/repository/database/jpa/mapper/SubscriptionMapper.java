...
public class SubscriptionMapper {
    ...
    public Subscription toDomain(SubscriptionEntity entity) {
        ...
        List<Tag> tags = entity.tags().stream().map(tagMapper::toDomain).toList();
        sub.tags(tags);

        List<Filter> filters =
                entity.filters().stream().map(filterMapper::toDomain).toList();
        sub.filters(filters);
    ...
    public SubscriptionEntity toEntity(Subscription domain) {
        ...
        entity.tags(domain.tags().stream().map(tagMapper::toEntity).toList());
        entity.filters(domain.filters().stream().map(filterMapper::toEntity).toList());
