...
public class JdbcTagRepository implements TagRepository {
    ...
    private static final String SELECT_BY_ID = "SELECT * FROM scrapper.tags WHERE id = ?";
    private static final String SELECT_BY_VALUE = "SELECT * FROM scrapper.tags WHERE tag = ?";
    private static final String INSERT_SQL = "INSERT INTO scrapper.tags (tag) VALUES (?) RETURNING " + BASE_COLUMNS;
    private static final String DELETE_BY_ID_SQL = "DELETE FROM scrapper.tags WHERE id = ?";
    ...
    public Optional<Tag> findById(TagId tagId) {
        List<Tag> list = jdbcTemplate.query(SELECT_BY_ID, rowMapper, tagId.id());
        return Optional.ofNullable(DataAccessUtils.singleResult(list));
    }

    @Override
    public Optional<Tag> findByTag(String tag) {
        List<Tag> list = jdbcTemplate.query(SELECT_BY_VALUE, rowMapper, tag);
        return Optional.ofNullable(DataAccessUtils.singleResult(list));
    }
    ...
    public Tag save(String tag) {
        return jdbcTemplate.queryForObject(INSERT_SQL, rowMapper, tag);
    }
    ...
    public Tag deleteById(TagId tagId) {
        ...
        jdbcTemplate.update(DELETE_BY_ID_SQL, tagId.id());
        return tag;
    }
}
