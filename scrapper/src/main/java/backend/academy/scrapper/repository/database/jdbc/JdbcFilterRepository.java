...
public class JdbcFilterRepository implements FilterRepository {
    private static final String SELECT_BY_ID = "SELECT id, filter, created_at FROM scrapper.filters WHERE id = ?";
    ...
    private static final String DELETE_BY_ID_SQL = "DELETE FROM scrapper.filters WHERE id = ?";
    ...
    public Optional<Filter> findById(FilterId filterId) {
        List<Filter> list = jdbcTemplate.query(SELECT_BY_ID, rowMapper, filterId.id());
        return Optional.ofNullable(DataAccessUtils.singleResult(list));
    }

    @Override
    public Optional<Filter> findByFilter(String filter) {
        List<Filter> list = jdbcTemplate.query(SELECT_BY_VALUE, rowMapper, filter);
        return Optional.ofNullable(DataAccessUtils.singleResult(list));
    }
    ...
    public Filter save(String filter) {
        return jdbcTemplate.queryForObject(INSERT_SQL, rowMapper, filter);
    }
    ...
    public Filter deleteById(FilterId filterId) throws NotExistFilterException {
        ...
        jdbcTemplate.update(DELETE_BY_ID_SQL, filterId.id());
        return filter;
    }
}
