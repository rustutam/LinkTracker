...
class LinkUpdateDetectionServiceTest {
    ...
    void getUpdatedLinksTest(){
        ...
        List<Subscription> subscriptions = List.of(TestModelFactory.createSubscription(), TestModelFactory.createSubscription());

        List<ChatId> expectedChatIds = subscriptions.stream().map(s -> s.user().chatId()).toList();
        ...
        assertThat(updatedLinks)
            ...
            .isEqualTo(expectedUpdatedLinks);
    }
